...
//    logger.addHandler(fh);
  ...
  private TTransport createUnderlyingTransport() throws TTransportException {
    ...
    if (isSslConnection()) {
      ...
      LOG.info("### Socket Transport with ssl set with loginTimeout:" + loginTimeout);
      LOG.error("### Socket Transport with ssl set with loginTimeout:" + loginTimeout);

      logger.info("### Socket Transport with ssl set with loginTimeout:" + loginTimeout);
    } else {
      // get non-SSL socket transport
      transport = HiveAuthUtils.getSocketTransport(host, port, loginTimeout);
      LOG.info("### Socket Transport without ssl set with loginTimeout:" + loginTimeout);
      LOG.error("### Socket Transport without ssl set with loginTimeout:" + loginTimeout);

      logger.info("### Socket Transport without ssl set with loginTimeout:" + loginTimeout);
    }
    return transport;
  }
  ...
  private void setupLoginTimeout() {
    ...
    LOG.info("###Set Socket timeout: " + socketTimeoutStr);
    LOG.error("###Set Socket timeout: " + socketTimeoutStr);

    logger.info("###Set Socket timeout: " + socketTimeoutStr);
    long timeOut = 0;
    try {
